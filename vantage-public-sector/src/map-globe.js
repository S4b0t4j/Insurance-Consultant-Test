...
(function (V) {
  ...
  V.globe.render = function (canvasHost, legendHost) {
    ...
    canvasHost.innerHTML =
      '<canvas id="globe-canvas" aria-label="Rotating globe of major public entities worldwide"></canvas>' +
      '<div class="globe-status" id="globe-status">Rotating. Drag the globe, or pick a country to spin and zoom to it.</div>';
    ...
    legendHost.innerHTML =
      '<h3>Countries</h3>' +
      '<div class="view-note" style="margin:0 0 10px">Showcase layer: real public entities at real city coordinates, display only. ' + V.modeledBadge() + '</div>' +
      '<div style="margin-bottom:10px"><button class="btn btn-ghost btn-sm" id="globe-reset">Resume rotation</button></div>' +
    ...
    function reset() {
      ...
      state.spinning = true;
      status.textContent = 'Rotating. Drag the globe, or pick a country to spin and zoom to it.';
      Array.prototype.forEach.call(legendHost.querySelectorAll('.globe-country'), function (b) {
    ...
    canvas.addEventListener('mousemove', function (ev) {
      ...
      if (hit) {
        ...
        V.tip.show('<b>' + V.fmt.esc(hit.city.entity) + '</b><div class="tip-sub">' +
          ...
          ' &middot; ' + (seg ? seg.label : hit.city.seg) + '</div>', ev.clientX, ev.clientY);
